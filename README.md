...
```bash
# 1. Bootstrap config in your project
synap init

# 2. Validate your entire setup
synap doctor

# 3. Browse available files in the remote repo
synap list

# 4. See the sync status of all tracked files
synap status

# 5. Pull everything down
synap pull

# 6. Pull a specific file (with tab completion)
synap pull copilot-instructions<TAB>

# 7. Pull interactively - choose files from a checklist
synap pull --interactive

# 8. See what changed upstream vs your local files
synap diff

# 9. Pull only files that have changed
synap update

# 10. Delete a tracked file
synap delete summarizer

# 11. Install shell tab completion (only needed if skipped during init)
...
### `synap init`
Interactively create a `synap.config.json`. Validates your GitHub token on setup and offers to install shell tab completion.
